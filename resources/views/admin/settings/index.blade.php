...
@section('content-header')
  <h1>Panel Settings<small>Configure OpenPanel to your liking.</small></h1>
  <ol class="breadcrumb">
              ...
              <div class="form-group col-md-4">
                ...
                <div>
                  <div class="btn-group" data-toggle="buttons">
                    @php
                      $level = old('openpanel:auth:2fa_required', config('openpanel.auth.2fa_required'));
                    @endphp
                    <label class="btn btn-outline-primary @if ($level == 0) active @endif">
                      <input type="radio" name="openpanel:auth:2fa_required" autocomplete="off" value="0" @if ($level == 0) checked @endif> Not Required
                    </label>
                    <label class="btn btn-outline-primary @if ($level == 1) active @endif">
                      <input type="radio" name="openpanel:auth:2fa_required" autocomplete="off" value="1" @if ($level == 1) checked @endif> Admin Only
                    </label>
                    <label class="btn btn-outline-primary @if ($level == 2) active @endif">
                      <input type="radio" name="openpanel:auth:2fa_required" autocomplete="off" value="2" @if ($level == 2) checked @endif> All Users
                    </label>
                  </div>
